...
import { dateToUnixTimestamp, processProfile } from '../utils';
...
export class PyroscopeApiExporter implements ProfileExporter {
  ...
  private handleAxiosError(error: AxiosError): void {
    if (error.response !== undefined) {
      ...
      Logger.log('Pyroscope received error while ingesting data to server');
      Logger.log(error.response.data);
    } else if (error.request !== undefined) {
      // The request was made but no response was received
      // `error.request` is an instance of XMLHttpRequest in the browser and an instance of
      // http.ClientRequest in node.js
      Logger.log('Error when ingesting data to server:', error.message);
    } else {
